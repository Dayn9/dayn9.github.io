...
// pre-load the images
PIXI.loader.
add([path + "images/GridSquare.png", path +"images/Treat.png", path +"images/Cheese.png", path +"images/Danger.png", path +"images/Needle.png", path +"images/Shock.png", path +"images/Gas.png", path +"images/Rat.png", path +"images/Heart.png", path +"images/CheesPlain.png", path +"images/HeartAnim.png", path +"images/RatIdle.png"]).
...
function setup() {
    ...
    //setup gas
    for(let g = 0; g < 9; g++){
        preGas.push(new Danger(0,0));
        gas.push(new Gas(0,0));
    }
    
    //setup sounds
    backgroundMusic = new Howl({
        src: [path + 'sounds/Background.wav'],
        autoplay: true,
        loop: true,
        volume: 0.5,
    });
    biteSound = new Howl({
	   src: [path + 'sounds/Bite.wav']
    });
    needleSound = new Howl({
        src: [path + 'sounds/Needle.wav'],
        volume: 3,
    });
    shockSound = new Howl({
	   src: [path + 'sounds/Shock.wav']
        
    });
    gasSound = new Howl({
        src: [path + 'sounds/Gas.wav']
    });
...
function createLabelsAndButtons(){
    ...
    trialCountdown.x = sceneWidth/2 - 150;
    trialCountdown.y = 10;
...
function clearBoard(){
    ...
    //remove gas
    gasTimer = 0;
    for(let g = 0; g<gas.length; g++){
        grid[(gas[g].gridX * 10) + gas[g].gridY].empty = true;
        gameScene.removeChild(gas[g]);
        gas[g].isAlive = false;  
        grid[(preGas[g].gridX * 10) + preGas[g].gridY].empty = true;
        gameScene.removeChild(preGas[g]);
        preGas[g].isAlive = false;  
    }
}
...
function gameLoop(){
    ...
    if(trial >= 3){
        ...
        if(gasVisible){
        ...
        }else if(preGas[0].isAlive && gasTimer > gasDangerTime){
            for(let g = 0; g < gas.length; g++){
                gas[g].move(preGas[g].x, preGas[g].y, preGas[g].gridX, preGas[g].gridY);
                gameScene.addChild(gas[g]);
                preGas[g].isAlive = false;
                gameScene.removeChild(preGas[g]);
                ratOnTop();
                gasVisible = true;
            ...
            gasTimer = 0;
        }else if(!preGas[0].isAlive && gasTimer > gasTime){
            spawnRandomGas();
        }
    }
...
//spawn a gas in a random grid 3x3 section 
function spawnRandomGas(){
    if(!(preGas[0].isAlive || gas[0].isAlive)){
        let sqrs = getEmptySection();
        if(sqrs != null){
            for(let g = 0; g < sqrs.length; g++){
                preGas[g].move(sqrs[g].x, sqrs[g].y, sqrs[g].gridX, sqrs[g].gridY);
                gameScene.addChild(preGas[g]);
            }
...
function getEmptySection(){
    ...
    do{
        if(tries > 20){return null;}
        row = Math.floor(Math.random() * (gridWidth - 2)) + 1;
        col = Math.floor(Math.random() * (gridHeight - 2) + 1);
        emptySqrs = 0;
        for(let r =  row-1; r <= row + 1; r++){
            for(let c = col -1; c <= col + 1; c++){
                if(grid[(r*10) + c].empty){
                    emptySqrs++;
                }
            }
        }
    ...
    for(let r =  row-1; r <= row + 1; r++){
        for(let c = col -1; c <= col + 1; c++){
            grid[(r*10) + c].empty = false;
            gridsquares.push(grid[(r*10) + c]);
        }
    }   
    return gridsquares;
}
...
function loadSpriteSheet(){
    let spriteSheet = PIXI.BaseTexture.fromImage(path + "images/RatIdle.png");
    let width = 64;
    let height = 64;
    let numFrames = 4;
    let textures = [];
...
function loadHeartSpriteSheet(){
    let spriteSheet = PIXI.BaseTexture.fromImage(path + "images/HeartAnim.png");
    let width = 20;
    let height = 18;
    let numFrames = 4;
    let textures = [];
